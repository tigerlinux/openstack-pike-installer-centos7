...
then
	...
	yum install -y openstack-gnocchi-indexer-sqlalchemy
	yum install -y openstack-gnocchi-statsd
...
then
	...
	then
		...
		cat ./libs/aodh/api-paste.ini > /etc/aodh/api-paste.ini
...
then
	...
	chown gnocchi.gnocchi /var/log/gnocchi/*.log
	...
	cp /usr/bin/gnocchi-api /var/www/cgi-bin/gnocchi/app.wsgi
