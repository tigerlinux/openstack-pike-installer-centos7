...
echo ""
echo "Creating GNOCCHI identities"
echo ""

echo "Gnocchi User:"
openstack user create --domain $keystonedomain --password $gnocchipass --email $gnocchiemail $gnocchiuser

echo "Gnocchi Role:"
openstack role add --project $keystoneservicestenant --user $gnocchiuser $keystoneadminuser

echo "Gnocchi Service:"
openstack service create \
        --name $gnocchisvce \
        --description "OpenStack Metric" \
        metric

echo "Gnocchi Endpoints:"

openstack endpoint create --region $endpointsregion \
	metric public http://$gnocchihost:8041

openstack endpoint create --region $endpointsregion \
	metric internal http://$gnocchihost:8041

openstack endpoint create --region $endpointsregion \
	metric admin http://$gnocchihost:8041
