...
#
# In order to have our cake and eat it too, let's do a first install pass without
# EPEL repo enabled: 
#
...
# Now, let's apply versionlock to zeromq package - there is a conflict between EPEL and RDO:
yum -y install yum-plugin-versionlock
yum versionlock zeromq*

# And with the zeromq version locked up, then do a second pass for all neutron packages, EPEL enabled:
...
then
	...
	#
	su -s /bin/sh -c "neutron-db-manage --config-file /etc/neutron/neutron.conf \
		--config-file /etc/neutron/plugin.ini --subproject neutron-fwaas upgrade head" neutron

	# Just a little failsafe in order to ensure proper lbaas v2 installation:
	su -s /bin/sh -c "neutron-db-manage --config-file /etc/neutron/neutron.conf \
		--config-file /etc/neutron/plugin.ini --subproject neutron-lbaas upgrade head" neutron

	# And for vpnaas too
	# su -s /bin/sh -c "neutron-db-manage --config-file /etc/neutron/neutron.conf \
	#	--config-file /etc/neutron/plugin.ini --subproject neutron-vpnaas upgrade head" neutron
fi
