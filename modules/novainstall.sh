...
then
	...
	cat ./libs/placement/00-nova-placement-api.conf > /etc/httpd/conf.d/00-nova-placement-api.conf
	sed -r -i "s/Listen\ 8778/Listen\ $novahost:8778/g" /etc/httpd/conf.d/00-nova-placement-api.conf
else
	echo ""
...
crudini --set /etc/nova/nova.conf keystone_authtoken region_name $endpointsregion
...
#
# New safe timeouts for Cinder block devices
#
...
crudini --set /etc/nova/nova.conf DEFAULT transport_url rabbit://$brokeruser:$brokerpass@$messagebrokerhost:5672/$brokervhost
# Deprecated !
...
echo ""
echo "ALERT !!.. PLEASE READ CAREFULLY (THIS MESSAGE WILL BE ON SCREEN FOR 20 SECONDS)"
